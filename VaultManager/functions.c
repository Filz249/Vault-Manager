...
void createDweller(DWELLER** dwellers, int* count) {
    ...
    *dwellers = temp;
    ...
    while (1) {
        ...
        if (!fgets(newDweller->firstName, MAX_NAME, stdin)) continue;
    ...
    while (1) {
        ...
        if (!fgets(newDweller->lastName, MAX_NAME, stdin)) continue;
    ...
    while (1) {
        ...
        if (scanf("%d", &newDweller->id) == 1 && newDweller->id > 0) break;
    ...
    while (1) {
        ...
        if (scanf("%f", &newDweller->health) == 1 && newDweller->health >= 0 && newDweller->health <= 100.00) break;
        printf("Invalid health. Must be 0-100%%.\n");
        clearInputBuffer();
    }

    // Input for S.P.E.C.I.A.L attributes
...
void updateDweller(DWELLER* dwellers, int count) {
    ...
    for (int i = 0; i < count; i++) {
        if (dwellers[i].id == id) {
            ...
            // Reset attributes and allow user to enter all points anew
            int points = MAX_POINTS;
            for (int j = 0; j < 7; j++) {
